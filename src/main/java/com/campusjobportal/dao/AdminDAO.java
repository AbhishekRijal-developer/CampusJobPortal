...
public class AdminDAO {
    ...
    public Map<String, Integer> getDashboardStats() {
        ...
        String keys[] = {"totalStudents", "totalRecruiters", "activeJobs", "totalApplications", "pendingRecruiters", "pendingJobs", "selectedCandidates", "pendingApplications"};
    ...
    /**
     * Get monthly application counts for the current year (for bar chart)
     * @return Map of month number (1-12) to application count
     */
    public Map<Integer, Integer> getMonthlyApplicationData() {
        Map<Integer, Integer> monthlyData = new HashMap<>();
        // Initialize all months to 0
        for (int i = 1; i <= 12; i++) {
            monthlyData.put(i, 0);
        }
        String query = "SELECT MONTH(application_date) as month, COUNT(*) as count " +
                       "FROM applications " +
        ...
        try (Connection conn = DBConnection.getConnection();
             ...
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                monthlyData.put(rs.getInt("month"), rs.getInt("count"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    /**
     * Get monthly hiring (APPROVED applications) counts for hiring trends line chart
     * @return Map of month number (1-12) to hired count
     */
    public Map<Integer, Integer> getMonthlyHiredData() {
        Map<Integer, Integer> hiredData = new HashMap<>();
        for (int i = 1; i <= 12; i++) {
            hiredData.put(i, 0);
        }
        String query = "SELECT MONTH(updated_date) as month, COUNT(*) as count " +
                       "FROM applications " +
                       "WHERE status = 'APPROVED' AND YEAR(updated_date) = YEAR(CURDATE()) " +
        ...
        try (Connection conn = DBConnection.getConnection();
             ...
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                hiredData.put(rs.getInt("month"), rs.getInt("count"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    /**
     * Get job counts by employment type for pie chart
     * @return Map of employment_type to count
     */
    public Map<String, Integer> getJobCategoryBreakdown() {
        Map<String, Integer> categories = new HashMap<>();
        String query = "SELECT employment_type, COUNT(*) as count FROM jobs GROUP BY employment_type ORDER BY count DESC";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                ...
                if (type != null && !type.isEmpty()) {
                    categories.put(type, rs.getInt("count"));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    /**
     * Get breakdown of applications by status for Doughnut chart
     * @return Map of status to count
     */
    public Map<String, Integer> getApplicationStatusBreakdown() {
        Map<String, Integer> statusBreakdown = new HashMap<>();
        String query = "SELECT status, COUNT(*) as count FROM applications GROUP BY status";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                ...
                if (status != null && !status.isEmpty()) {
                    statusBreakdown.put(status, rs.getInt("count"));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
